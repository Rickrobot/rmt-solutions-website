 ...
 *   (add the new price to the product, archive the old one) and these pages
 *   follow within about ten minutes.
 *
 ...
 *
 * WHEN IT IS READ
 *   At build time, and every five minutes after that. A failure is not an
 *   error — the price may genuinely not be set up yet — so callers must handle
 *   null and read perfectly well without a number in them.
 */
...
export async function planFor(product) {
  try {
    /* Five minutes, the same as the application's own cache of the figure.
       A price is now changed in Stripe and nowhere else, and an hour of this
       page quoting last week's price beside a card screen charging this
       week's is an hour too long. */
